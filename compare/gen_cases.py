...
# ================= saturation probe (previously covered ground) =================
add("p1", "{{ 1 + 2 * 3 }} {{ 2 ** 3 }} {{ 10 % 3 }} {{ 7 // 2 }}")
add("p2", "{{ 'abc' | upper }} {{ 'ABC' | lower }} {{ 'a b' | title }}")
add("p3", "{{ [3,1,2] | sort | join(',') }} {{ [1,2,3] | sum }} {{ [1,2] | first }}")
add("p4", "{{ d.a }} {{ d['b'] }} {{ d.c | default('x') }}", {"d": {"a": 1, "b": 2}})
add("p5", "{% for i in [1,2,3] %}{{ loop.index }}{{ i }}{% endfor %}")
add("p6", "{% if 1 < 2 %}y{% else %}n{% endif %}")
add("p7", "{{ x ~ y }}", {"x": "a", "y": 1})
add("p8", "{{ [1,2] | join('-') }} {{ {'a': 1} | length }}")
add("p9", "{% set x = 5 %}{{ x + 1 }}")
add("p10", "{{ 'a,b' | replace(',', '-') }}")
add("p11", "{{ [1,2,3] | reverse | join(',') }} {{ [2,1] | min }} {{ [2,1] | max }}")
add("p12", "{{ '%s=%d' | format('x', 1) }}")
add("p13", "{{ 'x' | default('d') }} {{ missing | default('d') }}")
add("p14", "{% for k, v in d.items() %}{{ k }}{{ v }}{% endfor %}", {"d": {"a": 1}})
add("p15", "{{ 'abc' | truncate(5, true, '...', 0) }}|{{ 'abc' | truncate(2, true, '...', 0) }}")
add("p16", "{{ '  x  ' | trim }}|{{ 'a-b-c' | split('-') | last }}")
add("p17", "{{ 1 is odd }} {{ 2 is even }} {{ none is none }} {{ 1 is number }}")
add("p18", "{{ range(3) | join(',') }} {{ range(1, 4, 2) | join(',') }}")
add("p19", "{{ {'b': 2, 'a': 1} | dictsort | map('first') | join(',') }}")
add("p20", "{{ [1,2] | select('odd') | join(',') }} {{ [1,2] | reject('odd') | join(',') }}")
add("p21", "{% macro m(a) %}{{ a }}!{% endmacro %}{{ m(1) }}")
add("p22", "{{ '<x>' | escape }} {{ ('<x>' | safe) }}")
add("p23", "{{ 1.5 | round }} {{ 2.5 | round }} {{ -1.5 | round }}")
add("p24", "{{ 'abc' | length }} {{ [1] | length }} {{ {'a': 1} | length }}")
add("p25", "{% for i in [1,2] recursive %}{{ i }}{% endfor %}")
add("p26", "{{ x if x else 'd' }}", {"x": 0})
add("p27", "{{ users | map(attribute='n') | join(',') }}", {"users": [{"n": "a"}, {"n": "b"}]})
add("p28", "{{ ['a','A','b'] | unique | join(',') }}")
add("p29", "{{ 1 in [1,2] }} {{ 'a' in 'abc' }} {{ 'k' in {'k': 1} }}")
add("p30", "{{ 'a b c'.split() | join('|') }}")
add("p31", "{{ 10 / 4 }} {{ 10 // 4 }} {{ 10 % 4 }}")
add("p32", "{{ true and 1 }} {{ false or 2 }}")
add("p33", "{{ ['b','a'] | sort | join(',') }}")
add("p34", "{% with a = 1 %}{{ a }}{% endwith %}")
add("p35", "{{ 'hello world' | wordcount }} {{ 'hi' | capitalize }}")
add("p36", "{{ [1,[2,3]] }} {{ {'a': [1]} }}")
add("p37", "{{ 5 | filesizeformat }} {{ 1048576 | filesizeformat }}")
add("p38", "{{ 'a' | urlencode }} {{ 'a b' | urlencode }}")
add("p39", "{{ {'a': [1,2]} | tojson }}")
add("p40", "{% filter upper %}x{% endfilter %}")
add("p41", "{{ [1,2] | batch(2) | map('join', '+') | join('|') }}")
add("p42", "{{ 'x' * 3 }} {{ [0] * 2 }}")
add("p43", "{{ (1, 2, 3)[1] }} {{ (1, 2) | length }}")
add("p44", "{% include 'p.html' %}", templates={"p.html": "P"})
add("p45", "{{ 'abc'.upper() }} {{ 'ABC'.lower() }}")
add("p46", "{{ 1 == 1.0 }} {{ '1' == 1 }} {{ none == none }}")
add("p47", "{{ [1,2] | map('string') | join(',') }}")
add("p48", "{% for i in [1,2] %}{% for j in [3] %}{{ i }}{{ j }}{% endfor %}{% endfor %}")
add("p49", "{{ not 0 }} {{ not 'a' }} {{ not none }}")
add("p50", "{{ 'abc' | list | join(',') }} {{ (1,2) | list | join(',') }}")
add("p51", "{{ 1234 | round(-2) }} {{ 1234 | abs }}")
add("p52", "{% block a %}X{% endblock %}")
add("p53", "{{ '<p>a</p> b' | striptags }}")
add("p54", "{{ 'a\nb' | indent(2) }}|")
add("p55", "{{ [1,2] | unique | sort | join(',') }}")
add("p56", "{{ true is sameas true }} {{ [] is iterable }} {{ [] is sequence }}")
add("p57", "{{ 'x' if true else 'y' }}")
add("p58", "{{ 1.0 }} {{ -0.0 }} {{ 1e3 }}")
...
add("range string step error", "{{ range(1, 3, '2') | join(',') }}")
add("range extra arg error", "{{ range(1, 3, 1, 9) | join(',') }}")
add("macro keyword expansion", "{% macro m(a, b=0) %}{{ a }}|{{ b }}|{{ kwargs['x'] }}{% endmacro %}{{ m(**d) }}", {"d": {"a": 1, "b": 2, "x": 3}})
add("map keyword filter", "{{ [1,2,3] | map(filter='string') | join(',') }}")
add("select keyword test", "{{ [1,2,3,4] | select(test='odd') | join(',') }}")
add("boolean true false tests", "{{ true is true }} {{ false is true }} {{ none is true }} {{ false is false }} {{ true is not false }}")
add("default alias", "{{ missing | d('fallback') }} {{ missing | d }}")
add("singleton random", "{{ [1] | random }} {{ 'a' | random }} {{ [7] | random }}")
add("include fallback with context", "{% include ['missing.html', 'p.html'] with context %}", {"x": 7}, templates={"p.html": "{{ x }}"})
add("include fallback without context", "{% include ['missing.html', 'p.html'] without context %}", {"x": 7}, templates={"p.html": "{{ x | default('missing') }}"})
add("from import with context", "{% from 'm.html' import v with context %}{{ v }}", {"x": 7}, templates={"m.html": "{% set v = x %}"})
add("from import without context", "{% from 'm.html' import v without context %}{{ v }}", {"x": 7}, templates={"m.html": "{% set v = x %}"})
add("urlize options", "{{ 'http://x.com' | urlize(target='_blank', rel='nofollow') }}")
add("slice fill keyword", "{% for col in [1,2,3] | slice(2, fill_with='-') %}[{{ col | join(',') }}]{% endfor %}")
add("sort case sensitive", "{{ ['b', 'A'] | sort(case_sensitive=true) | join(',') }}")
add("comparison test aliases", "{{ 1 is equalto 1 }} {{ 1 is == 1 }} {{ 1 is != 2 }} {{ 1 is lessthan 2 }} {{ 2 is greaterthan 1 }}")
